...
package com.domainsurvey.crawler.utils.robots;
...
public class SimpleRobotRules extends BaseRobotRules {
    ...
    public static class RobotRule implements Comparable<RobotRule>, Serializable {
        ...
        public boolean equals(Object obj) {
            ...
            } else return _prefix.equals(other._prefix);
        }
    }
    ...
    public RobotRule isAllowed(String url) {
        if (_mode == RobotRulesMode.ALLOW_NONE) {
            ...
            for (RobotRule rule : _rules) {
                boolean isRuleMatches = ruleMatches(pathWithQuery, rule._prefix);
                if (isRuleMatches) {
                    return rule;
                }
            }
        ...
        while ((patternPos < patternEnd) && (textPos < textEnd)) {
            ...
            if (wildcardPos == patternPos) {
                ...
                while ((textPos + patternPieceLen <= textEnd) && !matched) {
                    ...
                    matched = true;
                    for (int i = 0; i < patternPieceLen; i++) {
    ...
    public boolean equals(Object obj) {
        ...
        if (_mode != other._mode)
            return false;
        if (_rules == null) {
            return other._rules == null;
        } else return _rules.equals(other._rules);
    }
