...
@Component
public class ExternalLinkPaginator extends LinkPaginator {
    ...
    protected SelectJoinStep<? extends Record> baseQuery(DomainPaginationRequest domainPaginationRequest, SelectJoinStep<? extends Record> query) {
        String nodeTableName = TableService.getFullTableName(domainPaginationRequest.getId(), SchemaType.FINAL, TableType.NODE);
        String pageTableName = TableService.getFullTableName(domainPaginationRequest.getId(), SchemaType.FINAL, TableType.PAGE);

        query
                .innerJoin(table(nodeTableName).as(TableType.NODE.getValue()))
        ...
        query.where(String.format(
                ...
                TableType.EDGE.getValue(), id, TableType.NODE.getValue(), NodeType.EXTERNAL.getValue(), additionalDataWhere
        ));
